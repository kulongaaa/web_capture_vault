...
<img src="chrome-extension/cover.png" alt="网页学习助手封面" width="300"/>
...
1. **状态检查** - 点击插件图标，确认显示"本地知识应用 - 在线"
2. **收集网页** - 在任意网页点击"学习此网页知识"按钮
3. **管理知识** - 通过本地服务 API 管理知识内容
...
|------|------|------|
| `GET /health` | 健康检查 | - |
| `GET /api/notes` | 获取知识列表 | `?page=1&limit=20&search=关键词` |
| `POST /api/notes` | 创建知识 | `{title, content, url?, tags?}` |
| `PUT /api/notes/:id` | 更新知识 | `{title?, content?, tags?}` |
| `DELETE /api/notes/:id` | 删除知识 | - |
| `GET /api/search` | 搜索知识 | `?q=关键词` |
...
**服务器响应**
```javascript
{"success": true, "data": {"id": "note-uuid"}, "message": "知识创建成功"}
```
